...
Server::Server() {
	...
	search_your_reviews_sql = "SELECT review_id, R.user_id, R.movie_id, review_text, your_score, written, last_update, title, tmdb_score, user_avg_score, poster_path, username"
						...
						" AND user_id = :u_id"
						" ORDER BY title DESC";
	search_all_reviews_sql = "SELECT review_id, R.user_id, R.movie_id, review_text, your_score, written, last_update, title, tmdb_score, user_avg_score, poster_path, username"
						" FROM Movies M JOIN Reviews R ON (M.movie_id = R.movie_id) JOIN Users U ON (U.user_id = R.user_id)"
						" WHERE LOWER(title) LIKE LOWER(:search_term)"
						" OR LOWER(review_text) LIKE LOWER(:search_term)"
						" ORDER BY title DESC";
